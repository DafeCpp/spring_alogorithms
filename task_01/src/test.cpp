...
TEST(FindValuesTest, NegativeNumbers) {
    ...
    EXPECT_EQ(FindValues(0, 6, v), std::make_pair(-5, 5));  // Should this be (-5,5)? Actually 5 isn't in array
    // Fixed: test with actual pairs that exist
    EXPECT_EQ(FindValues(-2, 6, v), std::make_pair(-2, 0));
...
// Test where multiple pairs exist - should return first found pair (smallest left element)
...
TEST(FindValuesTest, UnsortedInput) {
    std::vector<int> v = {3, 1, 4, 2, 5};  // Unsorted, but algorithm assumes sorted!
    // Should work anyway? Two-pointer requires sorted array
    // EXPECT_EQ(FindValues(5, 5, v), std::make_pair(1, 4)); // Might fail if not sorted
}
